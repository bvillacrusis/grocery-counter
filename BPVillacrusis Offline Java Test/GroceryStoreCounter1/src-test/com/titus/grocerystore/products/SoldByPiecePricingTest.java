...
public class SoldByPiecePricingTest {
	...
	public static void displayTestDescription(){
		...
		System.out.println("\tThe following scenario shows a simple grocery checkout counter" +
				"\n\twherein a product SOLD BY PIECE will be scanned and the total "
	...
	public void generateSampleListOfProducts(){
		...
		for(int i=1; i<=numberOfDifferentPoductsAvailable; i++){
			...
			double randomPrice =Math.round((Math.random()*10000))/100.00;
		...
		for(int i=1; i<=20;i++){
			...
			preComputedTotalPrice += soldProduct.getPrice(); // to be used for testing the computed price
		}
	...
	public void scanItems() {
		...
		System.out.println("\t\t\t\t Item Price ($) \tTotal Price ($)");
